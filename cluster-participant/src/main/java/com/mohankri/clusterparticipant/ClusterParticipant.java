...
public class ClusterParticipant {
  private final String _zkAddr;
  private final String _clusterName;
  private final String _serverId;
  private HelixManager _manager = null;
  
  public ClusterParticipant(String zkAddr, String clusterName, String serverId) {
    _zkAddr = zkAddr;
    ...
    try {
      _manager =
          HelixManagerFactory.getZKHelixManager(_clusterName, _serverId, InstanceType.PARTICIPANT,
              _zkAddr);
      System.out.println("_clusterName " + _clusterName);
  ...
  public static void main(String[] args) throws Exception {
    if (args.length < 2) {
      System.err
          .println("USAGE: java ClusterParticipant zookeeperAddress(e.g. localhost:2181) serverId(host_port)");
      System.exit(1);
    }

    final String zkAddr = args[0];
    final String clusterName = CreateSetup.DEFAULT_CLUSTER_NAME;
    final String serverId = args[1];

    ZkClient zkclient = null;
    try {
      // start consumer
      final ClusterParticipant store = new ClusterParticipant(zkAddr, clusterName, serverId);
